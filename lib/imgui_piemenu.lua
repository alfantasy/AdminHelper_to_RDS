...
local c_posX, c_posY = getCursorPos()
...
local function BeginPiePopup(menuCtx, pName, iMouseButton)
	...
	if imgui.IsPopupOpen(pName) then
		...
		imgui.SetNextWindowPos( ImVec2( -100, -100 ), imgui.Cond.Always )
		...
		if bOpened then
			...
			if menuCtx.m_iLastFrame < (iCurrentFrame - 1) then
				...
				menuCtx.m_oCenter = ImVec2(getCursorPos())
			end
...
local function EndPiePopup(menuCtx)
	...
	local oMousePos = ImVec2(getCursorPos())
	local oDragDelta = ImVec2(oMousePos.x - c_posX, oMousePos.y - c_posY)
	...
	for iIndex = 0, menuCtx.m_iMaxIndex do
		...
		for item_n = 0, oPieMenu.m_iCurrentIndex - 1 do
			...
			for iSeg = 0, arc_segments do
				...
				local pos = ImVec2(c_posX + fCosInner * (fMinRadius + oStyle.ItemInnerSpacing.x), c_posY + fSinInner * (fMinRadius + oStyle.ItemInnerSpacing.x))
				local pos2 = ImVec2(c_posX + fCosOuter * (fMaxRadius - oStyle.ItemInnerSpacing.x), c_posY + fSinOuter * (fMaxRadius - oStyle.ItemInnerSpacing.x))
			...
			local oOuterCenter = ImVec2( c_posX + math.cos( fRadCenter ) * fMaxRadius, c_posY + math.sin( fRadCenter ) * fMaxRadius )
			...
			if oPieMenu.m_oItemIsSubMenu[item_n] then
				...
				oTrianglePos[ 0+1 ].x = c_posX + math.cos( fRadCenter ) * ( fMaxRadius - 5 )
				oTrianglePos[ 0+1 ].y = c_posY + math.sin( fRadCenter ) * ( fMaxRadius - 5 )
				oTrianglePos[ 1+1 ].x = c_posX + math.cos( fRadLeft ) * ( fMaxRadius - 10 )
				oTrianglePos[ 1+1 ].y = c_posY + math.sin( fRadLeft ) * ( fMaxRadius - 10 )
				oTrianglePos[ 2+1 ].x = c_posX + math.cos( fRadRight ) * ( fMaxRadius - 10 )
				oTrianglePos[ 2+1 ].y = c_posY + math.sin( fRadRight ) * ( fMaxRadius - 10 )
			...
			local text_pos = ImVec2(
				c_posX + math.cos((item_inner_ang_min + item_inner_ang_max) * 0.5) * (fMinRadius + fMaxRadius) * 0.5 - text_size.x * 0.5,
				c_posY + math.sin((item_inner_ang_min + item_inner_ang_max) * 0.5) * (fMinRadius + fMaxRadius) * 0.5 - text_size.y * 0.5)
