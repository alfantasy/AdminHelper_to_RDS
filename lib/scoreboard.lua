...
script_description('ImGui SA:MP Scoreboard Modded Yamada.')
...
EXPORTS = {
	version = 1,
	text_version = "v1.0 Relise (ScoreBoard)"
}
...
}, "..\\config\\AH_Setting\\scoreboard.ini")
local groups = inicfg.load({
	friend = {},
	admin = {},
	enemy = {}
}, "..\\config\\AH_Setting\\playergroupscoreboard.ini")
	...
	if show_main_window.v then
		if show_set_window.v then
			...
			if imgui.CollapsingHeader(u8"Общие настройки") then
				...
				if imgui.Combo("##type", cType, {u8"Маленький", u8"Средний", u8"Большой", u8"На весь экран"}) then
			...
			if imgui.Button(u8"Сохранить изменения", imgui.ImVec2(212, 0)) then
				...
				inicfg.save(allset, "..\\config\\AH_Setting\\scoreboard.ini")
				--[[if notf then
...
function drawScoreboardPlayer(id)
	...
	if id ~= getLocalPlayerId() and imgui.BeginPopupContextItem() then
		...
		if imgui.Combo("##cSetGroup", cSetGroup, {u8"Без группы", u8"Друг", u8"Админ", u8"Враг"}) then
			...
			inicfg.save(groups, "..\\config\\AH_Setting\\playergroupscoreboard.ini")
			imgui.CloseCurrentPopup()
		end
		imgui.PopItemWidth()
		imgui.EndChild()
...
function EXPORTS.ScriptData(scdata)
	script_ah = scdata
end
