...
module.BufferingBar = function(label, value, size_arg, bg_col, fg_col)
    ...
    local circleStart = size.x * 1;
    local circleEnd = size.x;
    local circleWidth = circleEnd - circleStart;
    
    DrawList:AddRectFilled(pos, imgui.ImVec2(pos.x + size.x, pos.y + size.y), bg_col)
    DrawList:AddRectFilled(pos, imgui.ImVec2(pos.x + size.x * value, pos.y + size.y), fg_col)
